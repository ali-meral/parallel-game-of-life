...
void exchange_boundaries(int n_loc_r, int n_loc_c, uint8_t (*current)[n_loc_c], MPI_Comm cartcomm) {
    ...
    printf("Rank %d received bottom right: ", rank);
    for (int i = 0; i < 4; i++) {
        printf("%d ", recv_bottom_right[i]);
    }
    printf("\n");

    printf("Rank %d received bottom left: ", rank);
    for (int i = 0; i < 4; i++) {
        printf("%d ", recv_bottom_left[i]);
    }
    printf("\n");

    printf("Rank %d received top right: ", rank);
    for (int i = 0; i < 4; i++) {
        printf("%d ", recv_top_right[i]);
    }
    printf("\n");

    printf("Rank %d received top left: ", rank);
    for (int i = 0; i < 4; i++) {
