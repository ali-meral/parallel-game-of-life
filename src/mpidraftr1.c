...
void exchange_boundaries(int n_loc_r, int n_loc_c, uint8_t (*current)[n_loc_c], MPI_Comm cartcomm) {
    ...
    // Update the current matrix with received data
    for (int i = 0; i < n_loc_r; i++) {
        current[i][0] = recv_left[i];
        current[i][1] = recv_left[i + n_loc_r];
        current[i][n_loc_c - 1] = recv_right[i];
        current[i][n_loc_c - 2] = recv_right[i + n_loc_r];
    }

    for (int j = 0; j < n_loc_c; j++) {
        current[0][j] = recv_top[j];
        current[1][j] = recv_top[j + n_loc_c];
        current[n_loc_r - 1][j] = recv_bottom[j];
        current[n_loc_r - 2][j] = recv_bottom[j + n_loc_c];
    }
