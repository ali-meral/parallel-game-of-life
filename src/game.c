...
void run_parallel(int argc, char *argv[])
{
    ...
    gather_ranks(n_loc_r, n_loc_c, n, rank, size, prow_idx * n_loc_r, pcol_idx * n_loc_c,
            current, global_matrix_par, cartcomm, verbose, iterations);
...
void run_collectives(int argc, char *argv[])
{
    ...
    // use collectives_communicate function
    collectives_communicate(n_loc_r, n_loc_c, current, extended_matrix, cartcomm);
